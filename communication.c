...
int send_request(tlv_request_t *user_request) { 
    ...
    if (write(fdr, user_request, sizeof(op_type_t) + sizeof(uint32_t) + user_request->length) <= 0) { return RC_SRV_DOWN; }
    if (close(fdr) != 0) return RC_SRV_DOWN;
    return RC_OK;
}
...
int get_request(tlv_request_t *user_request, int fd_log, int fd_srv) {   
    ...
    if (logRequest(fd_log, 0, user_request) < 0) { printf("Failed to open and write into %s\n", SERVER_LOGFILE); return RC_OTHER; }
    return RC_OK;
}
...
void *get_reply_thread(void *arg) {
    ...
    if (read(fda, &(thread_arg->reply.type), sizeof(enum op_type)) != sizeof(enum op_type)) { perror("get_reply_thread type"); return (void *)RC_OTHER; }
    if (read(fda, &(thread_arg->reply.length), sizeof(uint32_t)) != sizeof(uint32_t)) { perror("get_reply_thread lenght"); return (void *)RC_OTHER; }
    if (read(fda, &(thread_arg->reply.value), thread_arg->reply.length) != thread_arg->reply.length) { perror("get_reply_thread value"); return (void *)RC_OTHER; }
    if (close(fda) != 0) { thread_arg->completed = 1; return (void *)RC_OTHER; }
    unlink(fifo_reply);
    free(fifo_reply);
